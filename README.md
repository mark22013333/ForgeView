...
### External-Tomcat 路徑推導重寫（v0.9.0 新增）
- `findModuleRoot()` 從 WAR 路徑反推 module 根目錄（最多上找 6 層）
- Gradle 單模組打 WAR（`build/libs/*.war`）正確推導出專案根與 `./gradlew war`
- Maven 多模組 parent POM 自動偵測（Helpdesk4j 等多模組結構）
- `RunConfig.buildCwd` escape hatch：使用者可手動指定建置工作目錄，UI 含「Build 工作目錄」欄位
- `getDefaultCompileCommand` 偵測 parent POM 避免誤把 cwd 當 sub-module name
- WAR 路徑目錄 icon：自動偵測失敗時 fallback 到 native file/directory picker

### 遺留 Process 清理機制（v0.9.0 新增）
- PID 持久化至 `~/Library/Application Support/forgeview/active-services.json`
- 啟動時 reclaim：PID + start time 雙重驗證避免 PID 重用誤殺
- 接管後可正常使用「停止」「全部停止」按鈕殺掉前次 Tomcat / Java / Node process
- 服務 console 寫入接管提示，新 stdout 改參考 `.forgeview/logs/<id>.log`

### JDK Runtime 安全選擇（v0.9.0 新增）
- `spawnProcess` 強制套用服務設定的 `jdkId`，所有服務類型一致行為

### Service Config UX 全面重設計（v0.9.0 新增）
- 全新分區佈局：CoreForm / AdvancedSection / AutoDerivedFields / ModuleScanView / SharedJvmBar / BuildWarDropdown
- Context Path 三層推導：build 設定檔 → 歷史記憶 → WAR 檔名
- 共用 JVM 參數（專案層級 `sharedJvmArgs`）避免每個服務重複設定
- Build WAR 流程整合：UI dropdown 內直接觸發 Gradle/Maven WAR 建置

### webapp 路徑防呆（v0.9.0 新增）
- 新增 `service:validate-webapp-dir` IPC：偵測無 `WEB-INF` / `.jsp` / `.jspx` / `.html` / `.htm` / `.xhtml`
- 修預設值 bug：`watcherPath` 從錯誤的 `src/main/java` 改為標準 `src/main/webapp`
...
### v0.9.0（2026-05-24）
- External-Tomcat 路徑推導重寫：findModuleRoot 從 WAR 反推 module 根，正確支援 Gradle build/libs/*.war 與 Maven 多模組
- Maven 多模組 parent POM 自動偵測：cwd 指向 parent 目錄時不再誤把 cwd 當 sub-module name
- RunConfig.buildCwd escape hatch：使用者可手動指定建置工作目錄，UI 含「Build 工作目錄」欄位
- WAR 路徑目錄 icon 自動偵測失敗時 fallback 到 native file/directory picker
- 遺留 Process 清理機制：Forgeview 非正常關閉後重啟仍能停止前次的 Tomcat/Java/Node 服務（closes #9）
- PID + start time 雙重驗證避免 PID 重用誤殺，stale entry 自動清除
- JDK Runtime 安全選擇：避免 IntelliJ 啟動繼承 JBR JAVA_HOME 導致 Maven/Gradle 用錯 JDK
- Service Config UX 全面重設計：IntelliJ 風格 ServiceConfigModal，子元件分區、Context Path 三層推導、共用 JVM 參數
- webapp 路徑防呆：sourceWebappDir 不像 webapp 時 UI 提示，預設值修為 src/main/webapp
- checkWebappDir regex 涵蓋 .jspx / .xhtml，支援 JSF/Facelets 專案
- Release workflow 統一使用 ForgeView repo 名稱
